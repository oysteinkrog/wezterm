...
impl crate::TermWindow {
    pub fn paint_tab_bar(&mut self, layers: &mut TripleLayerQuadAllocator) -> anyhow::Result<()> {
        ...
        // Handle vertical tab bar (retro mode)
        if self.config.is_vertical_tab_bar() {
            ...
            let left_pixel_x = match position {
                TabBarPosition::Left => border.left.get() as f32,
            ...
            // Register UI items for click detection
            ...
            // Render each vertical line
            for (row_idx, line) in self.tab_bar.vertical_lines().iter().enumerate() {
                self.render_screen_line(
                    RenderScreenLineParams {
                        top_pixel_y: border.top.get() as f32 + (row_idx as f32 * cell_height),
                        left_pixel_x,
                        ...
                        dims: &RenderableDimensions {
                            cols: self.config.vertical_tab_width,
                            physical_top: 0,
                            scrollback_rows: 0,
                            scrollback_top: 0,
                            viewport_rows: 1,
        ...
        // Horizontal tab bar (original logic)
    ...
    /// Returns the pixel width of the vertical tab bar.
    /// Returns 0 if the tab bar is horizontal or not shown.
    pub fn tab_bar_pixel_width(&self) -> f32 {
        if self.config.is_vertical_tab_bar() {
            (self.config.vertical_tab_width * self.render_metrics.cell_size.width as usize) as f32
        } else {
            0.
        }
    }
